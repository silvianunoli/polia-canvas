...
const SYSTEM = `Voce e a Polia. Devolva o painel de numeros que importam: tres numeros-chave para acompanhar, ritmo recomendado de verificacao e o gatilho principal que pede atencao. Portugues do Brasil, sem emojis, sem traco longo.`

const TOOL = {
  type: 'function',
  function: {
    name: 'gerar_painel_numeros',
    description: 'Retorna o painel com 3 numeros, ritmo e gatilho.',
    parameters: {
      type: 'object',
      properties: {
        numero_1: { type: 'string', description: 'Primeiro numero-chave (nome + descricao curta).' },
        numero_2: { type: 'string', description: 'Segundo numero-chave (nome + descricao curta).' },
        numero_3: { type: 'string', description: 'Terceiro numero-chave (nome + descricao curta).' },
        ritmo_recomendado: { type: 'string', description: 'Com que ritmo olhar para esses numeros.' },
        gatilho_principal: { type: 'string', description: 'O sinal principal que pede atencao imediata.' },
      },
