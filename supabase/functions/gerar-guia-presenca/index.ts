...
const SYSTEM = `Voce e a Polia. Devolva o guia de primeira impressao da marca: canal principal, aparencia recomendada, caminho resumido que a cliente faz ate comprar e uma sugestao de bio curta. Portugues do Brasil, sem emojis, sem traco longo.`

const TOOL = {
  type: 'function',
  function: {
    name: 'gerar_guia_presenca',
    description: 'Retorna o guia de primeira impressao.',
    parameters: {
      type: 'object',
      properties: {
        canal_principal: { type: 'string', description: 'Canal principal recomendado e por que.' },
        aparencia_guia: { type: 'string', description: 'Como a presenca deve aparecer (tom, ritmo, visual).' },
        caminho_resumido: { type: 'string', description: 'Caminho da cliente em ate 3 passos claros.' },
        bio_sugerida: { type: 'string', description: 'Bio curta pronta para usar no canal principal.' },
      },
