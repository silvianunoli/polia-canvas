...
export const Route = createFileRoute("/_authenticated/etapa/3")({
  ...
  beforeLoad: async () => {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("etapa_atual, star_3_completed_at, star_2_completed_at")
      .eq("id", uid)
      .maybeSingle();
    if (!profile) return;
    // Acessivel se a etapa anterior foi concluida (ou se essa etapa ja foi concluida — modo leitura)
    const anteriorOk = !!(profile as Record<string, unknown>)["star_2_completed_at"];
    const essaOk = !!(profile as Record<string, unknown>)["star_3_completed_at"];
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 py-10 md:py-16 text-center">
        ...
        {!loading && !error && mapa && (
          <>
            ...
            <p className="caveat-informacional text-polia-terracota mt-4">olha onde você fica.</p>
