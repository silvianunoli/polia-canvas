...
export const Route = createFileRoute("/_authenticated/etapa/4")({
  ...
  beforeLoad: async () => {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("etapa_atual, star_4_completed_at, star_3_completed_at")
      .eq("id", uid)
      .maybeSingle();
    if (!profile) return;
    // Acessivel se a etapa anterior foi concluida (ou se essa etapa ja foi concluida — modo leitura)
    const anteriorOk = !!(profile as Record<string, unknown>)["star_3_completed_at"];
    const essaOk = !!(profile as Record<string, unknown>)["star_4_completed_at"];
