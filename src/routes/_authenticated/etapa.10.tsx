...
export const Route = createFileRoute("/_authenticated/etapa/10")({
  ...
  beforeLoad: async () => {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("etapa_atual, star_10_completed_at, star_9_completed_at")
      .eq("id", uid)
      .maybeSingle();
    if (!profile) return;
    // Acessivel se a etapa anterior foi concluida (ou se essa etapa ja foi concluida — modo leitura)
    const anteriorOk = !!(profile as Record<string, unknown>)["star_9_completed_at"];
    const essaOk = !!(profile as Record<string, unknown>)["star_10_completed_at"];
